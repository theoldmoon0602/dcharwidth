...
private bool	eastAsianWidth;
private Condition defaultCondition;

static this() {
  if (auto env = environment.get("RUNEWIDTH_EASTASIAN")){
...
bool inTables(A...)(dchar r, A tables) {
  foreach (t; tables) {
    if (r.inTable(t)) {
      return true;
    }
...
// dcharWidth returns the number of cells in r.
...
unittest {
  alias TestCaseT = Tuple!(dchar, "in_", int, "out_", int, "eaout");
  auto runewidthtests = [
    TestCaseT('世', 2, 2),
	  TestCaseT('界', 2, 2),
	  TestCaseT('ｾ', 1, 1),
	  TestCaseT('ｶ', 1, 1),
	  TestCaseT('ｲ', 1, 1),
	  TestCaseT('☆', 1, 2), // double width in ambiguous
	  TestCaseT('\x00', 0, 0),
	  TestCaseT('\x01', 0, 0),
	  TestCaseT('\u0300', 0, 0),
	  TestCaseT('\u2028', 0, 0),
  ...
  foreach (tt; runewidthtests) {
    auto out_ = c.dcharWidth(tt.in_);
    assert(out_ == tt.out_, "dcharWidth(%s) = %d, want %d".format(tt.in_, out_, tt.out_));
  }

	c.eastAsianWidth = true;
  foreach (tt; runewidthtests) {
    auto out_ = c.dcharWidth(tt.in_);
    assert(out_ == tt.eaout, "dcharWidth(%s) = %d, want %d".format(tt.in_, out_, tt.eaout));
  }
}

// stringWidth return width as you can see
int stringWidth(S)(Condition c, S s) {
  auto width = 0;
  foreach (dchar r; s) {
		width += c.dcharWidth(r);
  }
	return width;
}

unittest {
  alias TestCaseT = Tuple!(string, "in_", int, "out_", int, "eaout");
  auto stringwidthtests = [
    TestCaseT("■㈱の世界①", 10, 12),
	  TestCaseT("スター☆", 7, 8),
	  TestCaseT("つのだ☆HIRO", 11, 12),
  ];

	auto c = Condition();
	c.eastAsianWidth = false;
	foreach (tt; stringwidthtests) {
    auto out_ = c.stringWidth(tt.in_);
    assert(out_ == tt.out_, "stringWidth(%s) = %d, want %d".format(tt.in_, out_, tt.out_));
	}

	c.eastAsianWidth = true;
	foreach (tt; stringwidthtests) {
    auto out_ = c.stringWidth(tt.in_);
    assert(out_ == tt.eaout, "stringWidth(%s) = %d, want %d".format(tt.in_, out_, tt.eaout));
	}
}


// truncate return string truncated with w cells
S truncate(S)(Condition c, S s, int w, S tail) {
  import std.conv : to;
	...
	auto r = s.to!dstring;
	auto tw = c.stringWidth(tail);
	w -= tw;
	auto width = 0;
	auto i = 0;
	for (; i < r.length; i++) {
		auto cw = c.dcharWidth(r[i]);
		if (width+cw > w) {
			break;
		}
		width += cw;
	}
	return r[0..i].to!S ~ tail;
}

// wrap return string wrapped with w cells
S wrap(S)(Condition c, S s, int w) {
  import std.conv : to;
	auto width = 0;
	dchar[] out_ = [];
  foreach (dchar r; s) {
		auto cw = dcharWidth(r);
		if (r == '\n') {
			out_ ~= r;
			width = 0;
			continue;
...
S fillLeft(S)(Condition c, S s, int w) {
  import std.range : repeat, take;
  import std.conv : to;
	auto width = c.stringWidth(s);
	auto count = w - width;
	if (count > 0) {
    return ' '.repeat.take(count).to!S ~ s;
	}
	return s;
}

// fillRight return string filled in left by spaces in w cells
S fillRight(S)(Condition c, S s, int w) {
  import std.range : repeat, take;
  import std.conv : to;
	...
	auto count = w - width;
	if (count > 0) {
    return s ~ ' '.repeat.take(count).to!S;
	}
	return s;
}
...
int dcharWidth(dchar r) {
	return defaultCondition.dcharWidth(r);
}

// isAmbiguousWidth returns whether is ambiguous width or not.
bool isAmbiguousWidth(dchar r) {
	return inTables(r, private_, ambiguous);
}

unittest {
  alias TestCaseT = Tuple!(dchar, "in_", bool, "out_");
  auto isambiguouswidthtests = [
    TestCaseT('世', false),
	  TestCaseT('■', true),
	  TestCaseT('界', false),
	  TestCaseT('○', true),
	...
	foreach(tt; isambiguouswidthtests) {
    auto out_ = isAmbiguousWidth(tt.in_);
    assert(out_ == tt.out_, "isAmbiguousWidth(%s) = %s, want %s".format(tt.in_, out_, tt.out_));
	}
}
...
unittest {
  ...
  auto isneutralwidthtests = [
	...
	foreach(tt; isneutralwidthtests) {
    auto out_ = isNeutralWidth(tt.in_);
    assert(out_ == tt.out_, "isNeutralWidth(%s) = %s, want %s".format(tt.in_, out_, tt.out_));
  }
}

// stringWidth return width as you can see
int stringWidth(S)(S s) {
	return defaultCondition.stringWidth(s);
}

unittest {
	auto s = "こんにちわ\x00世界";
  auto out_ = stringWidth(s);
  assert(out_ == 14, "stringWidth(%s) = %d, want %d".format(s, out_, 14));
}

// truncate return string truncated with w cells
S truncate(S)(S s, int w, S tail) {
...
unittest {
	auto s = "あいうえお";
	auto expected = "あいうえお";
  auto out_ = truncate(s, 10, "...");

	assert(out_ == expected, "truncate(%s) = %s, want %s".format(s, out_, expected));
}

unittest {

	auto s = "あいうえおあいうえおえおおおおおおおおおおおおおおおおおおおおおおおおおおおおおお";
  auto expected = "あいうえおあいうえおえおおおおおおおおおおおおおおおおおおおおおおおおおおお...";

	auto out_ = truncate(s, 80, "...");
  assert(out_ == expected, "truncate(%s) = %s, want %s".format(s, out_, expected));

	auto width = stringWidth(out_);
  assert(width == 79, "width of truncate(%s) should be %d, but %d".format(s, 79, width));
}

unittest {
	auto s = "aあいうえおあいうえおえおおおおおおおおおおおおおおおおおおおおおおおおおおおおおお";
	auto expected = "aあいうえおあいうえおえおおおおおおおおおおおおおおおおおおおおおおおおおおお...";

	auto out_ = truncate(s, 80, "...");
	assert(out_ == expected, "truncate(%s) = %s, want %s".format(s, out_, expected));
	auto width = stringWidth(out_);
  assert(width == 80, "width of truncate(%s) should be %d, but %d".format(s, 80, width));
}

unittest {
	auto s = "あいうえおあいうえおえおおおおおおおおおおおおおおおおおおおおおおおおおおおおお";
	auto expected = "あいうえおあいうえおえおおおおおおおおおおおおおおおおおおおおおおおおおおおおお";
...
unittest {
	auto s = `東京特許許可局局長はよく柿喰う客だ/東京特許許可局局長はよく柿喰う客だ
123456789012345678901234567890
END`;
	auto expected = `東京特許許可局局長はよく柿喰う
客だ/東京特許許可局局長はよく
柿喰う客だ
123456789012345678901234567890
END`;
  auto out_ = wrap(s, 30);
	assert(out_ == expected, "wrap(%s) = %s, want %s".format(s, out_, expected));
}

// fillLeft return string filled in left by spaces in w cells
S fillLeft(S)(S s, int w) {
	return defaultCondition.fillLeft(s, w);
}

unittest {
	auto s = "あxいうえお";
	auto expected = "    あxいうえお";

  auto out_ = fillLeft(s, 15);
  assert(out_ == expected, "fillLeft(%s) = %s, want %s".format(s, out_, expected));
}
...
unittest {
	...
	auto out_ = fillLeft(s, 10);
  assert(out_ == expected, "fillLeft(%s) = %s, want %s".format(s, out_, expected));
}

// fillRight return string filled in left by spaces in w cells
S fillRight(S)(S s, int w) {
...
unittest {
  ...
  auto out_ = fillRight(s, 15);
  assert(out_ == expected, "fillRight(%s) = %s, want %s".format(s, out_, expected));
}
...
unittest {
	...
	auto out_ = fillRight(s, 10);
  assert(out_ == expected, "fillRight(%s) = %s, want %s".format(s, out_, expected));
}

unittest {
	auto old = environment.get("RUNEWIDTH_EASTASIAN", "");
	scope(exit) environment["RUNEWIDTH_EASTASIAN"] = old;

	environment["RUNEWIDTH_EASTASIAN"] = "1";

  auto w = dcharWidth('│');
  assert(w == 1, "RuneWidth('│') = %d, want %d".format(w, 1));
}

version(Posix) {
  import std.regex;

  private const reLoc = ctRegex!(r"^[a-z][a-z][a-z]?(?:_[A-Z][A-Z])?\.(.+)");
  ...
  bool isEastAsian(string locale) {
    ...
    auto charset = locale.toLower();
    auto r = matchAll(locale, reLoc);
    if (r.front.length == 2) {
      charset = r.front[1].toLower();
    }
    ...
    auto max = 1;
    if (auto m = charset in mblenTable) {
      max = *m;
    }

    if (max > 1 && (charset[0] != 'u' ||
          startsWith(locale, "ja") ||
          startsWith(locale, "ko") ||
          startsWith(locale, "zh"))) {
      return true;
    }
    return false;
  }
  ...
  unittest {
    alias TestCaseT = Tuple!(string, "locale", bool, "want");
    auto testcases = [
      TestCaseT("foo@cjk_narrow", false),
      TestCaseT("foo@cjk", false),
      TestCaseT("utf-8@cjk", false),
      TestCaseT("ja_JP.CP932", true),
    ];

    foreach (tt; testcases) {
      auto got = isEastAsian(tt.locale);
      assert(got == tt.want, "isEastAsian(%s) should be %s".format(tt.locale, tt.want));
    }
  }

  // isEastAsian return true if the current locale is CJK
  bool isEastAsian() {
    auto locale = environment.get("LC_CTYPE", environment.get("LANG", ""));

    // ignore C locale
    if (locale == "POSIX" || locale == "C") {
      return false;
    }
    if (locale.length > 1 && locale[0] == 'C' && (locale[1] == '.' || locale[1] == '-')) {
      return false;
    }
  ...
  unittest {
    auto lcctype = environment.get("LC_CTYPE", "");
    scope(exit) environment["LC_CTYPE"] = lcctype;

    alias TestCaseT = Tuple!(string, "lcctype", bool, "want");
    auto testcases = [
      TestCaseT("ja_JP.UTF-8", true),
      TestCaseT("C", false),
      TestCaseT("POSIX", false),
      TestCaseT("en_US.UTF-8", false),
    ];

    foreach (tt; testcases) {
      environment["LC_CTYPE"] = tt.lcctype;
      auto got = isEastAsian();
      assert(got == tt.want, "isEastAsian() for LC_CTYPE=%s should be %s".format(tt.lcctype, tt.want));
    }
  }
  ...
  unittest {
    ...
    auto testcases = [
      ...
      TestCaseT("POSIX", false),
      TestCaseT("en_US.UTF-8", false),
      TestCaseT("C.UTF-8", false),
    ];

    foreach (tt; testcases) {
      environment["LANG"] = tt.lcctype;
      auto got = isEastAsian();
      assert(got == tt.want, "isEastAsian() for LANG=%s should be %s".format(tt.lcctype, tt.want));
    }
  }
}

version(Windows) {
  pragma(msg, "Not implemented for Windows yet");
}
